...
if __name__ == "__main__":
    ...
    print(f"📍 Health: http://0.0.0.0:{config.PORT}/job/health")
    print(f"📍 Admin:  http://0.0.0.0:{config.PORT}/admin/health")  # FR9: NEW
    print("=" * 60)
    # ✅ FIX: pehle debug=True hardcoded tha (security risk in production).
    # Ab config.DEBUG se aata hai, jo .env / hosting platform ke
    # environment variable se control hota hai.
    # Note: Render/Railway production mein Gunicorn use karte hain,
    # is block ka asar sirf local "python app.py" chalane par hota hai.
